...
@push('scripts')
    ...
    <script type="text/javascript">
        ...
        $(document).on('click', '.categories_status', function() {
            var id = $(this).attr('data-id');
            var is_active = $(this).val();
            $.ajax({
                type: 'POST',
                url: route('product_category.update.status'),
                data: {
                    id: id,
                    is_active: is_active
                },
                success: function(data) {
                    ...
                    if (is_active == "1") {
                        $('.categories_status#switch' + id).attr('value', 0);
                    } else {
                        $('.categories_status#switch' + id).attr('value', 1);
                    }
        ...
        $(document).on("click", ".remove", function(event) {
            var flag = confirm('Are You Sure want to Remove Product Category?');
            if (flag) {
                var id = $(this).data('id');
                $.ajax({
