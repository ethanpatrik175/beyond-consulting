...
<script src="{{asset('/assets/backend/datatable/dataTables.bootstrap4.min.js')}}"></script>
    <script type="text/javascript">
        ...
        $(document).on('click', '.blogs_status', function() {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (is_active == "1") {
                        $('.blogs_status#switch' + id).attr('value', '0');
                    } else {
                        $('.blogs_status#switch' + id).attr('value', '1');
                    }
        ...
        $(document).on("click", ".remove", function(event) {
            var flag = confirm('Are You Sure want to Remove blogs?');
            if (flag) {
                var id = $(this).data('id');
                $.ajax({
