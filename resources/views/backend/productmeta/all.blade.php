    ...
    <script type="text/javascript">
        $(function() {
            var table = $('.data-table').DataTable({
                processing: true,
                serverSide: false,
                ajax: route('product_metas.index'),
                columns: [{
                    data: 'id',
                    name: 'id'
                },  {
        ...
        $(document).on('click', '.productmetas_status', function() {
            var id = $(this).attr('data-id');
            var is_active = $(this).val();
            $.ajax({
                type: 'POST',
                url: route('product_meta.update.status'),
                data: {
                    id: id,
                    is_active: is_active
                },
                success: function(data) {
                    ...
                    if (is_active == "1") {
                        $('.productmetas_status#switch' + id).attr('value', 0);
                    } else {
                        $('.productmetas_status#switch' + id).attr('value', 1);
                    }
        ...
        $(document).on("click", ".remove", function(event) {
            var flag = confirm('Are You Sure want to Remove Product Meta?');
            if (flag) {
                var id = $(this).data('id');
                $.ajax({
                    url: route('product_metas.destroy', id),
                    type: 'DELETE',
                    data: {
                        id: id
                    },
