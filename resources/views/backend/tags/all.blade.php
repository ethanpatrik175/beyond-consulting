...
@push('scripts')
    ...
    <script type="text/javascript">
        ...
        $(document).on('click', '.tags_status', function() {
            var id = $(this).attr('data-id');
            var is_active = $(this).val();
            $.ajax({
                type: 'POST',
                url: route('tag.update.status'),
                data: {
                    id: id,
                    is_active: is_active
                },
                success: function(data) {
                    ...
                    if (is_active == 1) {
                        $('.tags_status#switch' + id).attr('value', '0');
                    } else {
                        $('.tags_status#switch' + id).attr('value', '1');
                    }
        ...
        $(document).on("click", ".remove", function(event) {
            var flag = confirm('Are You Sure want to Remove Tags?');
            if (flag) {
                var id = $(this).data('id');
                $.ajax({
